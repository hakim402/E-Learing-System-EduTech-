...
if(isset($_SESSION['is_login'])){
    ...
    if(isset($_REQUEST['updateStuNameBtn'])){
        if(($_REQUEST['stuName'] == "")){
            ...
            $img_folder = '../image/Students/'. $stu_image;
            move_uploaded_file($stu_image_temp, $img_folder);

            $sql = "UPDATE student SET stu_name = '$stuName', stu_occ = '$stuOcc', stu_img = '$img_folder' WHERE stu_email = '$stuEmail'";
            if($conn->query($sql) == TRUE){
                $passmsg = '<div class="alert alert-success col-sm-6 ml-5 mt-2" rol="alert">Update Successfully</div>';
            } else{
...
<div class="col-sm-6 mt-5">
    <form class="mx-5" action="" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
            <label for="stuImg">Upload Image</label> <br>
